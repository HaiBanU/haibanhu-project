...
function saveCurrentUserToSession(user) { 
    if (!user) {
        sessionStorage.removeItem('haiBanhU_CurrentUser');
        return;
    }
    try {
        sessionStorage.setItem('haiBanhU_CurrentUser', JSON.stringify(user));
    } catch (e) {
        if (e.name === 'QuotaExceededError') {
            console.error("Lỗi QuotaExceededError: Không đủ dung lượng trong sessionStorage. Dữ liệu người dùng quá lớn.");
            showToast("Lỗi nghiêm trọng: Dữ liệu phiên quá lớn, không thể lưu. Vui lòng thử đăng xuất và đăng nhập lại.", "error", 10000);
        } else {
...
async function updateUserOnServer(specificUpdate = false) {
    if (!currentUser) return;
    try {
        let dataToUpdate;
        if (specificUpdate) {
            dataToUpdate = specificUpdate;
        } else {
             dataToUpdate = {
                ...
                workflows: currentUser.workflows,
                friends: currentUser.friends,
                friendRequests: currentUser.friendRequests,
                projectInvites: currentUser.projectInvites,
        ...
        showToast("Không thể lưu thay đổi trên máy chủ.", "error");
    }
}
...
async function sendFriendRequest(targetId) {
    ...
    try {
        ...
        if (button) {
            ...
            button.innerHTML = '<i class="fas fa-user-plus"></i> Thêm bạn bè';
        }
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupRealtimeListeners() {
        ...
        socket.on('project_updated', async (data) => {
            ...
            try {
                ...
                console.error("Không thể làm mới dữ liệu người dùng sau khi cập nhật:", err);
                return;
            }
    ...
    function setupGlobalListeners() {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            if (t.id === 'modal-backdrop' && document.querySelector('.dock-panel.is-open')) {
            } else if (t.id === 'modal-backdrop') {
                closeModal();
            }
            ...
            const handlersById = {
                ...
                '#back-to-dashboard-btn': () => {
                    const openPanel = document.querySelector('.dock-panel.is-open');
                    if (openPanel) {
